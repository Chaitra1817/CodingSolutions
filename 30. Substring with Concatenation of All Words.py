...
class Solution:
    def findSubstring(self, text: str, words: List[str]) -> List[int]:
        ...
        for start in range(len(text) - window_len + 1):
            window_text = text[start:start + window_len]

            seen_counts = Counter()
            for pos in range(0, len(window_text), word_len):
                current_word = window_text[pos:pos + word_len]
                seen_counts[current_word] += 1

            if seen_counts == required_counts:
